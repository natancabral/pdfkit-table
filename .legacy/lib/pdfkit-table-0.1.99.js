  ...
  table(table, options, callback) {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (options.useSafelyMarginBottom === undefined)
          options.useSafelyMarginBottom = true;
        ...
        const calcColumnSizes = () => {
          ...
          options.width &&
            (w =
              ...
              String(options.width).replace(/[^0-9]/g, "") >> 0);
        ...
        const addHeader = () => {
          ...
          if (!options.hideHeader) {
            ...
            rowBottomY = Math.max(
              startY + computeRowHeight(table.headers, true),
              rowBottomY,
            );
        ...
        table.datas.forEach((row, i) => {
          ...
          if (
            options.useSafelyMarginBottom &&
            this.y + safelyMarginBottom + rowHeight >= maxY &&
            !lockAddPage
          )
            onFirePageAdded(); // this.emitter.emit('addPage'); //this.addPage();
            ...
            if (typeof table.headers[index] === "object") {
              ...
              table.headers[index].renderer &&
                (cell = table.headers[index].renderer(
                  ...
                  rectCell,
                  this,
                )); // text-cell, index-column, index-line, row, doc[this]
              // align
