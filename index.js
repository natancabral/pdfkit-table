    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const maxY             = this.page.height - (this.page.margins.bottom); // this.page.margins.top +
          ...
          let safelyMarginBottom = this.page.margins.top/2 + (options.marginBottom ?? 0);
        ...
        table.datas.forEach((row, i) => {
          ...
          const tempY = this.y + safelyMarginBottom + rowHeight + options.marginBottom;
          if(options.useSafelyMarginBottom && tempY >= maxY && !lockAddPage) onFirePageAdded(); //
        ...
        table.rows.forEach((row, i) => {
          ...
          // else this.emitter.emit('addPage'); //this.addPage();
          const tempY = this.y + safelyMarginBottom + rowHeight + options.marginBottom;
          if(options.useSafelyMarginBottom && tempY >= maxY && !lockAddPage) onFirePageAdded(); // this.emitter.emit('addPage'); //this.addPage();
          ...
          row.forEach((cell, index) => {
            ...
            if(typeof table.headers[index] === 'object') {
              ...
              table.headers[index].fontName && (fontName = table.headers[index].fontName);
            }
